...
export async function handlerUploadPOST(req, res, folder) {
  try {
    ...
    if (!video || !coverImage || !categoryVideoID) {
      return res
        .status(400)
        .json({ error: "Missing required fields (video, coverImage, categoryVideoID)" });
    }

    // Benzersiz bir ID oluştur
    const id = uuidv4();

    // Video ve Cover Image yükleme
    ...
    // Firestore'da veri ekleme
    const newDoc = await addDoc(collection(db, "categoryVideoID"), {
      categoryVideoID: categoryVideoID[0],
      videoUrl,
      coverImageUrl,
      createdAt: new Date(),
    });
