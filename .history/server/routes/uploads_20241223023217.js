...
import { getFirestoreDoc } from "../configs/firebase";
...
export async function handlerUploadPOST(req, res) {
  try {
    ...
    // Firestore'dan Category Name'i Alma
    const categoryDoc = await getFirestoreDoc("categoryVideo", categoryId);
    if (!categoryDoc.exists) {
      res.status(404).json({ error: "Category not found." });
      return;
    }

    const { name: categoryName } = categoryDoc.data();

    // Firestore'a ekleme
    const mediaData = {
      id,
      categoryId,
      categoryName, // Kategori ismi buraya ekleniyor
      videoUrl,
      coverImageUrl,
