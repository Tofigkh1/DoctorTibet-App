...
export async function handlerUploadPOST(req, res, folder) {
  try {
    ...
    const { categoryVideoID } = form.fields; // categoryVideoID formdan gelen alan

    // Video ve kapak görseli kontrolü
    if (!video || video.length === 0 || !coverImage || coverImage.length === 0) {
      res.status(400).json({ error: "Video or cover image is missing" });
      return;
    }

    // categoryVideoID kontrolü
    if (!categoryVideoID) {
      res.status(400).json({ error: "CategoryVideoID is missing" });
      return;
    }
    ...
    // Firestore'de categoryVideo koleksiyonuna ekleme
    const categoryVideoDoc = {
      id, // Benzersiz video ID
      videoUrl,
      coverImageUrl,
      categoryVideoID, // Formdan gelen kategori ID'si
      createdAt: new Date().toISOString(), // Eklenme tarihi
    };

    await setDoc(doc(firestore, "categoryVideo", id), categoryVideoDoc);

    // Başarılı yanıt
    res.status(201).json({ id, videoUrl, coverImageUrl, categoryVideoID });
  } catch (error) {
