...
import { storage, firestore } from "../configs/firebase"; // Firebase yapılandırmanızın yolu
import { doc, setDoc } from "firebase/firestore"; // Firestore işlemleri için
...
export async function handlerUploadPOST(req, res, folder) {
  try {
    ...
    const { id } = form.fields; // Body'den ID alınır

    if (!id || typeof id !== "string") {
      res.status(400).json({ error: "ID is missing or invalid in the request body" });
      return;
    }
    ...
    // Firestore'da categoryVideo oluştur ve veriyi ekle
    const categoryVideoData = {
      id,
      videoUrl,
      coverImageUrl,
      createdAt: new Date().toISOString(), // Ekstra bilgi (isteğe bağlı)
    };

    const categoryVideoRef = doc(firestore, "categoryVideos", id); // ID body'den alınır
    await setDoc(categoryVideoRef, categoryVideoData);
