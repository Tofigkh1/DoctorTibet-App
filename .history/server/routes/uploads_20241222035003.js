...
import { firestore } from "../configs/firebase"; // Firestore yapılandırmanız
...
export async function handlerUploadPOST(req, res, folder) {
  try {
    ...
    const { categoryVideoID } = form.fields; // Gelen form alanlarından categoryVideoID'yi alıyoruz.

    if (!video || video.length === 0 || !coverImage || coverImage.length === 0 || !categoryVideoID) {
      res.status(400).json({ error: "Video, cover image, or categoryVideoID is missing" });
      return;
    }
    ...
    await firestore.collection("categoryVideos").doc(id).set(categoryVideoData);

    // Başarılı yanıt
    res.status(201).json(categoryVideoData);
  } catch (error) {
