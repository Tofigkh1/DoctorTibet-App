...
import { storage, firestore } from "../configs/firebase"; // Firestore yapılandırmanızı ekleyin
import { v4 as uuidv4 } from "uuid";

export async function handlerUploadPOST(req, res, folder) {
  try {
    // Form verilerini çözümle
    const form = await parseForm(req);
    const { video, coverImage } = form.files;
    const { categoryVideoID } = form.fields;

    if (!video || video.length === 0 || !coverImage || coverImage.length === 0 || !categoryVideoID) {
      res.status(400).json({ error: "Missing required fields (video, coverImage, categoryVideoID)" });
      return;
    }
    ...
    // Kapak görselini yükleme
    const uploadedCoverImage = coverImage[0];
    const coverImageUrl = await uploadFile(`${folder}/covers/${id}`, uploadedCoverImage);

    // Firestore'dan categoryVideo koleksiyonunda categoryVideoID'ye göre name alma
    const categoryVideoDocRef = doc(firestore, "categoryVideo", categoryVideoID);
    const categoryVideoDoc = await getDoc(categoryVideoDocRef);

    if (!categoryVideoDoc.exists()) {
      res.status(404).json({ error: "Category video not found" });
      return;
    }

    const { name } = categoryVideoDoc.data();

    // Yeni koleksiyona yazma
    const categoryVideoIDDocRef = doc(firestore, "categoryVideoID", id);
    await setDoc(categoryVideoIDDocRef, { name, categoryVideoID });

    // Başarılı yanıt
    res.status(201).json({ id, videoUrl, coverImageUrl, name });
  } catch (error) {
