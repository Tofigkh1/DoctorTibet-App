...
import { doc, setDoc, getDoc } from "firebase/firestore"; // Firestore'dan veri çekmek için
import { firestore } from "../configs/firebase";
import { v4 as uuidv4 } from "uuid";

export async function handlerUploadPOST(req, res, folder, categoryId) {
  try {
    ...
    // Firestore'dan kategori bilgisi alın
    const categoryRef = doc(firestore, "categories", categoryId); // Kategori koleksiyonu
    ...
    const categoryName = categoryData.name; // Kategorinin ismi
    ...
    const videoDocRef = doc(firestore, "videos", id); // "videos" koleksiyonuna ekle
    await setDoc(videoDocRef, {
      id,
      videoUrl,
      coverImageUrl,
      categoryId, // Kategori ID'si
      categoryName, // Firestore'dan alınan kategori ismi
      createdAt: new Date().toISOString(),
    });

    res.status(201).json({ id, videoUrl, coverImageUrl, categoryId, categoryName });
  } catch (error) {
