...
export async function handlerUploadPOST(req, res, folder) {
  try {
    ...
    // Kategori bağı ekleniyor
    const mediaData = {
      id,
      videoUrl,
      coverImageUrl,
      categoryId, // Firestore kategorileriyle ilişkilendirme için
      createdAt: new Date().toISOString(),
    };

    await addData("media", mediaData);
...
export async function handlerGetAllVideos(req, res, folder) {
  try {
    ...
    // Filtreleme
    const filteredMedia = all
      ? allMedia
      : allMedia.filter((media) => media.categoryId === categoryId);

    // Kategori bilgilerini almak için kategori koleksiyonunu getir
    const allCategories = await getAllData("categories");
    const mediaList = filteredMedia.map((media) => {
      const category = allCategories.find((cat) => cat.id === media.categoryId);
      return {
        ...media,
