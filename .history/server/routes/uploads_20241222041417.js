...
import { UploadCategory } from "../models/uploadCategory";

export async function handlerUploadPOST(req, res, folder,col) {
  try {
    const form = await parseForm(req);
    const { categoryVideoID } = req.body;
    ...
    if (!video || video.length === 0 || !coverImage || coverImage.length === 0) {
      ...
      return;
        ...
        const Upload = new UploadCategory(
          categoryVideoID
        ).toPlainObject();
    const data = await addData(col, Upload);
    const id = uuidv4();
    ...
    res.status(201).json({ id, videoUrl, coverImageUrl,data });
  } catch (error) {
