...
import { firestore } from "../configs/firebase"; // Firestore bağlantısını buradan al
  ...
  try {
    ...
    const categories = await Promise.all(
      categoryIds.map(async (id) => {
        const categoryRef = firestore.collection("categoryVideo").doc(id);
    ...
    await Promise.all(
      categories.map(async (category) => {
        console.log("categorybu",category);
