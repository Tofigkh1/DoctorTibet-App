...
export async function handlerUploadPOST(req, res, folder) {
  try {
    ...
    const { categoryVideoID } = form.fields; // Text alanları buradan alınır.
    ...
    // Firestore'a kategori videosu ekleme
    const categoryVideoData = {
      id: categoryVideoID, // Artık kullanıcıdan alınan ID kullanılıyor
      videoUrl,
      coverImageUrl,
      createdAt: new Date().toISOString(),
    };

    await addDoc(collection(firestore, "categoryVideos"), categoryVideoData);
