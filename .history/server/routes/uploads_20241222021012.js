...
import { listAll, getDownloadURL, ref, get } from "firebase/storage";
import { db, storage } from "../configs/firebase"; // Firebase yapılandırmanızın yolu
import { doc, getDoc } from "firebase/firestore"; // Firestore'dan getirme işlemi
...
export async function handlerUploadPOST(req, res, folder) {
  try {
    ...
    if (!categoryVideoId) {
      res.status(400).json({ error: "Category video ID is missing" });
      return;
    }

    // Kategori ismini veritabanından al
    const categoryDocRef = doc(db, "categoryVideos", categoryVideoId); // Firestore'daki koleksiyon adı
    ...
    const categoryVideoName = categoryDoc.data().name;
    ...
    res.status(201).json({ 
      ...
      videoUrl, 
      coverImageUrl, 
      categoryVideo: { id: categoryVideoId, name: categoryVideoName } 
    });
  } catch (error) {
