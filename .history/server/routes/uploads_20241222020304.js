...
export async function handlerUploadPOST(req, res, folder, categoryVideoId) {
  try {
    // Gelen `req` objesinden form verisini ayrıştırın
    ...
    if (!categoryVideoId) {
      res.status(400).json({ error: "CategoryVideo ID is missing" });
      return;
    }
    ...
    const videoUrl = await uploadFile(`${folder}/${categoryVideoId}/videos/${id}`, uploadedVideo);

    // Kapak görseli yükleme
    const uploadedCoverImage = coverImage[0];
    const coverImageUrl = await uploadFile(`${folder}/${categoryVideoId}/covers/${id}`, uploadedCoverImage);
