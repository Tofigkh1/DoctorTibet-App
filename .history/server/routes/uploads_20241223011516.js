...
export async function handlerUploadPOST(req, res, folder) {
  try {
    ...
    const { category } = form.fields; // Category alanını alıyoruz.

    if (!video || video.length === 0 || !coverImage || coverImage.length === 0 || !category) {
...
export async function handlerGetAllVideos(req, res, folder) {
  try {
    ...
    for (const videoFolder of videoFolders.prefixes) {
      ...
      // Kategoriyi ID'den türet
      const category = videoFolderName.split("_")[1] || "Uncategorized";
