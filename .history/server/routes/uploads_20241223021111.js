...
export async function handlerUploadPOST(req, res, folder) {
  try {
    ...
    const { categoryId } = form.fields; // Category ID buradan alınıyor.

    if (!video || !coverImage || !categoryId) {
      res.status(400).json({ error: "Video, cover image or categoryId is missing" });
      return;
    }
    ...
    // Firestore'a kategoriye bağlı video kaydı ekle.
    const videoData = {
      id,
      videoUrl,
      coverImageUrl,
    ...
    await addData("videos", videoData);
