...
export default async function handler(req, res) {
  ...
  try {
    switch (req.method) {
      case METHOD.POST: {
        const { categoryId } = req.query; // categoryId'yi sorgu parametresinden alıyoruz.

        if (!categoryId) {
          res.status(400).json({ error: "categoryId is required in query parameters" });
          return;
        }

        if (handlerUploadPOST && typeof handlerUploadPOST === "function") {
          await handlerUploadPOST(req, res, ROUTER.UPLOADS, categoryId);
        } else {
          res
            .status(500)
