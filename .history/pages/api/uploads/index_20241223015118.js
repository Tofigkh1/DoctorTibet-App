...
export default async function handler(req, res) {
  ...
  switch (req.method) {
    case METHOD.POST:
      ...
      // Eğer kategori oluşturulacaksa
      if (type === "category") {
        if (handlerCategoryPOST && typeof handlerCategoryPOST === "function") {
          await handlerCategoryPOST(req, res);
        } else {
          res
            .status(500)
            .json({ error: "handlerCategoryPOST function is not defined" });
        }
      } 
      // Eğer dosya (video ve cover) yüklenecekse
      else {
    ...
    case METHOD.GET:
      ...
      // Tüm dosyaları kategori bilgileriyle getir
      if (all && all === "true") {
      ...
      // Eksik parametreler için hata döndür
      else {
        res
          .status(400)
          .json({
